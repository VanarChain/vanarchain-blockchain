...
type priceQueue struct {
	baseFee *big.Int
	list    []*types.Transaction
	length  int // Track the length of the queue
}

func (h *priceQueue) cmp(a, b *types.Transaction) int {
	if h.baseFee != nil {
		// Compare effective tips if baseFee is specified
		if c := a.EffectiveGasTipCmp(b, h.baseFee); c != 0 {
			return c
...
func (q *priceQueue) Push(tx *types.Transaction) {
	q.list = append(q.list, tx)
	q.length++
}

func (q *priceQueue) Pop() *types.Transaction {
	if q.length == 0 {
		return nil
	}
	tx := q.list[0]
...
// underpricedFor checks whether a transaction is cheaper than (or as cheap as) the
// lowest priced (remote) transaction in the given heap.
func (l *pricedList) underpricedFor(h *priceQueue, tx *types.Transaction) bool {
	// Discard stale price points if found at the queue start
	for len(h.list) > 0 {
		head := h.list[0]
		if l.all.GetRemote(head.Hash()) == nil { // Removed or migrated
			l.stales.Add(-1)
			h.Pop() // Remove the first element from the queue
			continue
		}
		break
	}
...
func (l *pricedList) Discard(slots int, force bool) (types.Transactions, bool) {
	drop := make(types.Transactions, 0, slots) // Remote underpriced transactions to drop
	for slots > 0 {
		if l.urgent.Len()*floatingRatio > l.floating.Len()*urgentRatio {
			// Discard stale transactions if found during cleanup
			tx := l.urgent.Pop()
			if l.all.GetRemote(tx.Hash()) == nil { // Removed or migrated
				l.stales.Add(-1)
			...
			// Non-stale transaction found, move to floating queue
			l.floating.Push(tx)
		} else {
			if l.floating.Len() == 0 {
...
func (l *pricedList) Reheap() {
	...
	start := time.Now()
	l.stales.Store(0)
	urgentQueue := priceQueue{
		baseFee: l.urgent.baseFee,
		list:    []*types.Transaction{},
	}
	l.all.Range(func(hash common.Hash, tx *types.Transaction, local bool) bool {
		urgentQueue.Push(tx)
	...
	l.urgent = urgentQueue

	// balance out the two queues
	floatingCount := len(l.urgent.list) * floatingRatio / (urgentRatio + floatingRatio)
	floatingQueue := priceQueue{
		list: []*types.Transaction{},
	}
	for i := 0; i < floatingCount; i++ {
		floatingQueue.Push(l.urgent.Pop())
	}
	l.floating = floatingQueue
	reheapTimer.Update(time.Since(start))
}
