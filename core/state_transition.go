...
import (
	...
	cmath "github.com/TerraVirtuaCo/vanarchain-blockchain/common/math"
	"github.com/TerraVirtuaCo/vanarchain-blockchain/consensus/misc/eip4844"
...
func (st *StateTransition) buyGas() error {
	...
	if st.evm.ChainConfig().IsLahore(st.evm.Context.BlockNumber) {
		...
		if st.evm.ChainConfig().IsKarachi(st.evm.Context.BlockNumber) {
			if st.evm.ChainConfig().IsSialkot(st.evm.Context.BlockNumber) {
				feeMultiplier =  new(big.Int).SetUint64(st.feeTiersV3(new(big.Int).SetUint64(st.msg.GasLimit)))
			} else {
...
func (st *StateTransition) TransitionDb() (*ExecutionResult, error) {
	...
	if st.evm.Config.NoBaseFee && msg.GasFeeCap.Sign() == 0 && msg.GasTipCap.Sign() == 0 {
		...
		if st.evm.ChainConfig().IsLahore(st.evm.Context.BlockNumber) {
			...
			if st.evm.ChainConfig().IsKarachi(st.evm.Context.BlockNumber) {
				if st.evm.ChainConfig().IsSialkot(st.evm.Context.BlockNumber){
					feeMultiplier = new(big.Int).SetUint64(st.feeTiersV3(feeForGasUsed))
				} else {
...
func (st *StateTransition) refundGas(refundQuotient uint64) {
	...
	if st.evm.ChainConfig().IsLahore(st.evm.Context.BlockNumber) {
		...
		if st.evm.ChainConfig().IsKarachi(st.evm.Context.BlockNumber) {
			if st.evm.ChainConfig().IsSialkot(st.evm.Context.BlockNumber) {
				initialFeeMultiplier = new(big.Int).SetUint64(st.feeTiersV3(new(big.Int).SetUint64(st.initialGas)))
				usedFeeMultiplier = new(big.Int).SetUint64(st.feeTiersV3(new(big.Int).SetUint64(st.gasUsed())))
			} else {
